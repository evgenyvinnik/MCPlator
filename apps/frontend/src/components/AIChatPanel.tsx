  ...
  return (
    <div
      className={`relative h-full w-full bg-gradient-to-br from-blue-900 via-cyan-800 to-blue-800 shadow-2xl flex flex-col overflow-hidden ${
        isMobile ? `rounded-t-[28px] rounded-b-none ${isMinimized ? 'h-20' : ''}` : 'rounded-[28px]'
      } ${className}`}
    >
      {/* Background Effects */}
      ...
      <div className="relative z-20 flex items-center justify-between p-4 border-b border-white/10 backdrop-blur-lg bg-white/5">
        <div className="flex items-center gap-3">
          <div className={`${isMobile ? 'w-16 h-16' : 'w-10 h-10'} rounded-full bg-gradient-to-br from-cyan-400 to-blue-500 flex items-center justify-center shrink-0`}>
            <MessageSquare className={`${isMobile ? 'w-7 h-7' : 'w-5 h-5'} text-white`} />
          </div>
          <div>
          ...
          <div ref={chatContainerRef} className="flex-1 overflow-y-auto p-4 space-y-4 relative z-10">
            {messages.length === 0 && !streamingMessage && (
              <div className="text-center py-12 space-y-4">
                <div className={`${isMobile ? 'w-20 h-20' : 'w-16 h-16'} mx-auto rounded-full bg-gradient-to-br from-cyan-400/20 to-blue-500/20 flex items-center justify-center backdrop-blur-lg`}>
            ...
            {streamingMessage && (
              <div className="flex justify-start">
                <Card className={`max-w-[85%] ${isMobile ? 'p-4' : 'p-3'} backdrop-blur-lg border-0 shadow-xl bg-white/10 text-white border border-white/20`}>
                  <p className={isMobile ? 'text-base' : 'text-sm'}>{streamingMessage.text}</p>
                  {isThinking && (
